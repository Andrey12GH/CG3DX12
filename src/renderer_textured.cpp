...
RenderingSystem::~RenderingSystem()
{
    ...
    if (mLightingCB && mLightingCBMapped)
...
bool RenderingSystem::Initialize(HWND hwnd, int width, int height)
{
    try
    {
        ...
        CreateRenderTargets();
        CreateDepthStencil();
        mGBuffer.Initialize(mDevice.Get(), static_cast<UINT>(width), static_cast<UINT>(height));
        CreateFence();
...
void RenderingSystem::Update()
{
    ...
    mWorld = XMMatrixIdentity();
    mTextureOffset = XMFLOAT2(0.0f, 0.0f);
...
void RenderingSystem::BuildLights(float seconds)
{
    mLights.clear();
    ...
    XMStoreFloat3(
        ...
        XMVector3Normalize(XMVectorSet(0.35f, -1.0f, 0.25f, 0.0f)));
    directional.color = XMFLOAT3(0.76f, 0.82f, 1.0f);
    directional.intensity = 0.65f;
    mLights.push_back(directional);

    const std::array<XMFLOAT3, 6> pointPositions =
    {
        XMFLOAT3(-2.0f, -0.35f, -0.8f),
        XMFLOAT3(-0.8f, 0.45f, 0.65f),
        XMFLOAT3(0.65f, -0.2f, -0.3f),
        XMFLOAT3(1.9f, 0.5f, 0.85f),
        XMFLOAT3(-1.5f, 1.15f, 0.2f),
        XMFLOAT3(1.3f, 1.25f, -0.65f)
    };
    const std::array<XMFLOAT3, 6> pointColors =
    {
        XMFLOAT3(1.0f, 0.22f, 0.12f),
        XMFLOAT3(0.10f, 0.55f, 1.0f),
        XMFLOAT3(0.20f, 1.0f, 0.40f),
        XMFLOAT3(1.0f, 0.72f, 0.18f),
        XMFLOAT3(0.75f, 0.25f, 1.0f),
        XMFLOAT3(0.15f, 0.95f, 0.90f)
    };

    for (size_t index = 0; index < pointPositions.size(); ++index)
    {
        SceneLight point;
        point.type = LightType::Point;
        point.position = pointPositions[index];
        point.position.y += std::sin(seconds * 0.8f + static_cast<float>(index)) * 0.12f;
        point.range = 2.15f;
        point.color = pointColors[index];
        point.intensity = 5.0f;
        mLights.push_back(point);
    }

    const auto directionTo = [](const XMFLOAT3& from, const XMFLOAT3& target)
    {
        XMFLOAT3 direction;
        XMStoreFloat3(
            &direction,
            XMVector3Normalize(XMVectorSubtract(
                XMLoadFloat3(&target), XMLoadFloat3(&from))));
        return direction;
    };

    const std::array<XMFLOAT3, 3> spotPositions =
    {
        XMFLOAT3(-2.1f, 1.75f, -1.5f),
        XMFLOAT3(0.0f, 1.95f, 0.8f),
        XMFLOAT3(2.1f, 1.7f, -1.2f)
    };
    const std::array<XMFLOAT3, 3> spotTargets =
    {
        XMFLOAT3(-1.1f, -0.65f, 0.2f),
        XMFLOAT3(0.0f, -0.65f, 0.0f),
        XMFLOAT3(1.1f, -0.65f, 0.25f)
    };
    const std::array<XMFLOAT3, 3> spotColors =
    {
        XMFLOAT3(1.0f, 0.45f, 0.20f),
        XMFLOAT3(0.35f, 0.55f, 1.0f),
        XMFLOAT3(0.35f, 1.0f, 0.55f)
    };

    for (size_t index = 0; index < spotPositions.size(); ++index)
    {
        SceneLight spot;
        spot.type = LightType::Spot;
        spot.position = spotPositions[index];
        spot.direction = directionTo(spot.position, spotTargets[index]);
        spot.range = 4.25f;
        spot.spotInnerCos = std::cos(XMConvertToRadians(18.0f));
        spot.spotOuterCos = std::cos(XMConvertToRadians(28.0f));
        spot.color = spotColors[index];
        spot.intensity = 7.0f;
...
void RenderingSystem::CreateDescriptorHeaps()
{
    ...
    ThrowIfFailed(mDevice->CreateDescriptorHeap(
        &rtvDescription, IID_PPV_ARGS(&mBackBufferRtvHeap)), "Create RTV heap");
...
void RenderingSystem::CreateRenderTargets()
{
    D3D12_CPU_DESCRIPTOR_HANDLE handle =
        mBackBufferRtvHeap->GetCPUDescriptorHandleForHeapStart();
...
void RenderingSystem::CreateDepthStencil()
{
    D3D12_RESOURCE_DESC description{};
...
{
    const auto createRootSignature = [this](
        ...
        ComPtr<ID3D12RootSignature>& rootSignature)
    {
        ...
        ThrowIfFailed(mDevice->CreateRootSignature(
            ...
            IID_PPV_ARGS(&rootSignature)), operation);
    };
    ...
    D3D12_ROOT_PARAMETER geometryParameters[3]{};
    ...
    geometryParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
    geometryParameters[1].Descriptor.ShaderRegister = 1;
    geometryParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

    geometryParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    geometryParameters[2].DescriptorTable.NumDescriptorRanges = 1;
    geometryParameters[2].DescriptorTable.pDescriptorRanges = &materialSrvRange;
    ...
    D3D12_ROOT_SIGNATURE_DESC geometryDescription{};
    geometryDescription.NumParameters = _countof(geometryParameters);
    geometryDescription.pParameters = geometryParameters;
    geometryDescription.NumStaticSamplers = 1;
    geometryDescription.pStaticSamplers = &sampler;
    geometryDescription.Flags =
        D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;
    createRootSignature(
        geometryDescription,
        "Create geometry root signature",
        mGeometryRootSignature);

    D3D12_DESCRIPTOR_RANGE gbufferSrvRange{};
    gbufferSrvRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
    gbufferSrvRange.NumDescriptors = GBuffer::AttachmentCount;
    gbufferSrvRange.BaseShaderRegister = 0;
    gbufferSrvRange.OffsetInDescriptorsFromTableStart =
        D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
    ...
    lightingParameters[1].DescriptorTable.pDescriptorRanges = &gbufferSrvRange;
    lightingParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

    D3D12_ROOT_SIGNATURE_DESC lightingDescription{};
    lightingDescription.NumParameters = _countof(lightingParameters);
    lightingDescription.pParameters = lightingParameters;
    createRootSignature(
        lightingDescription,
        "Create lighting root signature",
        mLightingRootSignature);
}

void RenderingSystem::BuildPipelineStateObjects()
{
    const ComPtr<ID3DBlob> geometryVertexShader =
        CompileShader(Shaders::GeometryVertexShader, "vs_5_0");
    const ComPtr<ID3DBlob> geometryPixelShader =
        CompileShader(Shaders::GeometryPixelShader, "ps_5_0");
    const ComPtr<ID3DBlob> lightingVertexShader =
        CompileShader(Shaders::LightingVertexShader, "vs_5_0");
    const ComPtr<ID3DBlob> lightingPixelShader =
        CompileShader(Shaders::LightingPixelShader, "ps_5_0");
    ...
    D3D12_RENDER_TARGET_BLEND_DESC geometryTargetBlend{};
    geometryTargetBlend.RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
    D3D12_BLEND_DESC geometryBlend{};
    for (UINT index = 0; index < GBuffer::AttachmentCount; ++index)
        geometryBlend.RenderTarget[index] = geometryTargetBlend;
    ...
    geometryDescription.InputLayout = { layout, _countof(layout) };
    geometryDescription.pRootSignature = mGeometryRootSignature.Get();
    geometryDescription.VS = {
        geometryVertexShader->GetBufferPointer(), geometryVertexShader->GetBufferSize() };
    geometryDescription.PS = {
        geometryPixelShader->GetBufferPointer(), geometryPixelShader->GetBufferSize() };
    geometryDescription.RasterizerState = rasterizer;
    geometryDescription.BlendState = geometryBlend;
    ...
    geometryDescription.NumRenderTargets = GBuffer::AttachmentCount;
    geometryDescription.RTVFormats[0] = GBuffer::GetFormat(GBuffer::AlbedoSpecular);
    geometryDescription.RTVFormats[1] = GBuffer::GetFormat(GBuffer::Normal);
    geometryDescription.RTVFormats[2] = GBuffer::GetFormat(GBuffer::WorldPosition);
    geometryDescription.DSVFormat = DXGI_FORMAT_D32_FLOAT;
    geometryDescription.SampleDesc.Count = 1;

    ThrowIfFailed(mDevice->CreateGraphicsPipelineState(
        &geometryDescription, IID_PPV_ARGS(&mGeometryPipelineState)),
        "Create geometry pipeline state");

    D3D12_RASTERIZER_DESC lightingRasterizer = rasterizer;
    lightingRasterizer.CullMode = D3D12_CULL_MODE_NONE;

    D3D12_RENDER_TARGET_BLEND_DESC additiveTargetBlend{};
    additiveTargetBlend.BlendEnable = TRUE;
    additiveTargetBlend.SrcBlend = D3D12_BLEND_ONE;
    additiveTargetBlend.DestBlend = D3D12_BLEND_ONE;
    additiveTargetBlend.BlendOp = D3D12_BLEND_OP_ADD;
    additiveTargetBlend.SrcBlendAlpha = D3D12_BLEND_ONE;
    additiveTargetBlend.DestBlendAlpha = D3D12_BLEND_ZERO;
    additiveTargetBlend.BlendOpAlpha = D3D12_BLEND_OP_ADD;
    additiveTargetBlend.RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
    D3D12_BLEND_DESC additiveBlend{};
    additiveBlend.RenderTarget[0] = additiveTargetBlend;
    ...
    lightingDescription.pRootSignature = mLightingRootSignature.Get();
    lightingDescription.VS = {
        lightingVertexShader->GetBufferPointer(), lightingVertexShader->GetBufferSize() };
    lightingDescription.PS = {
        lightingPixelShader->GetBufferPointer(), lightingPixelShader->GetBufferSize() };
    lightingDescription.RasterizerState = lightingRasterizer;
    lightingDescription.BlendState = additiveBlend;
    lightingDescription.DepthStencilState = noDepth;
    lightingDescription.SampleMask = UINT_MAX;
    lightingDescription.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
    lightingDescription.NumRenderTargets = 1;
    lightingDescription.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;
    lightingDescription.SampleDesc.Count = 1;

    ThrowIfFailed(mDevice->CreateGraphicsPipelineState(
        &lightingDescription, IID_PPV_ARGS(&mLightingPipelineState)),
        "Create lighting pipeline state");
}

void RenderingSystem::BuildObj(const std::string& path)
{
...
void RenderingSystem::BuildTextureResources()
{
    ...
    ThrowIfFailed(mDevice->CreateDescriptorHeap(
        &heapDescription, IID_PPV_ARGS(&mMaterialSrvHeap)), "Create SRV heap");
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE srvHandle =
        mMaterialSrvHeap->GetCPUDescriptorHandleForHeapStart();
...
void RenderingSystem::BuildConstantBuffers()
{
    ...
    D3D12_RESOURCE_DESC lightingBuffer = BufferDescription(
        std::max<size_t>(1, mLights.size()) * ConstantBufferAlignment);
    ThrowIfFailed(mDevice->CreateCommittedResource(
        &uploadHeap, D3D12_HEAP_FLAG_NONE, &lightingBuffer,
        D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
        IID_PPV_ARGS(&mLightingCB)), "Create lighting constant buffer");
    ThrowIfFailed(mLightingCB->Map(
        0, nullptr, reinterpret_cast<void**>(&mLightingCBMapped)),
...
void RenderingSystem::RenderGeometryPass(D3D12_CPU_DESCRIPTOR_HANDLE dsv)
{
    mCommandList->SetPipelineState(mGeometryPipelineState.Get());
    mGBuffer.BeginGeometryPass(mCommandList.Get(), dsv);

    mCommandList->SetGraphicsRootSignature(mGeometryRootSignature.Get());
    ID3D12DescriptorHeap* heaps[] = { mMaterialSrvHeap.Get() };
    ...
    const D3D12_GPU_DESCRIPTOR_HANDLE heapStart =
        mMaterialSrvHeap->GetGPUDescriptorHandleForHeapStart();
    for (const ObjSubset& subset : mModel.subsets)
    {
        ...
        mCommandList->SetGraphicsRootConstantBufferView(
            1, mMaterialCB->GetGPUVirtualAddress() +
            static_cast<UINT64>(subset.materialIndex) * ConstantBufferAlignment);

        D3D12_GPU_DESCRIPTOR_HANDLE textureHandle = heapStart;
        textureHandle.ptr +=
            static_cast<UINT64>(material.textureSrvIndex) * mSrvDescriptorSize;
        mCommandList->SetGraphicsRootDescriptorTable(2, textureHandle);
        mCommandList->DrawIndexedInstanced(
            subset.indexCount, 1, subset.startIndex, 0, 0);
    }

    mGBuffer.EndGeometryPass(mCommandList.Get());
}

void RenderingSystem::RenderLightingPass(
    D3D12_CPU_DESCRIPTOR_HANDLE backBufferRtv)
{
    const FLOAT clearColor[] = { 0.012f, 0.018f, 0.032f, 1.0f };
    mCommandList->ClearRenderTargetView(backBufferRtv, clearColor, 0, nullptr);
    mCommandList->OMSetRenderTargets(1, &backBufferRtv, TRUE, nullptr);

    mCommandList->SetPipelineState(mLightingPipelineState.Get());
    mCommandList->SetGraphicsRootSignature(mLightingRootSignature.Get());
    ID3D12DescriptorHeap* heaps[] = { mGBuffer.GetShaderVisibleHeap() };
    mCommandList->SetDescriptorHeaps(1, heaps);
    mCommandList->SetGraphicsRootDescriptorTable(1, mGBuffer.GetGpuSrvStart());
    ...
    {
        const SceneLight& light = mLights[index];
        LightingConstants constants{};
        constants.lightPosition = light.position;
        constants.lightRange = light.range;
        constants.lightDirection = light.direction;
        constants.spotInnerCos = light.spotInnerCos;
        constants.lightColor = light.color;
        constants.intensity = light.intensity;
        constants.cameraPosition = mCameraPosition;
        constants.spotOuterCos = light.spotOuterCos;
        constants.lightType = static_cast<uint32_t>(light.type);
        constants.ambientStrength = index == 0 ? 0.055f : 0.0f;

        std::memcpy(
            mLightingCBMapped + index * ConstantBufferAlignment,
            &constants,
            sizeof(constants));

        mCommandList->SetGraphicsRootConstantBufferView(
            0,
            mLightingCB->GetGPUVirtualAddress() +
            static_cast<UINT64>(index) * ConstantBufferAlignment);
